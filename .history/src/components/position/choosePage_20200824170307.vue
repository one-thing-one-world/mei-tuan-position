       ...
       <el-input  @focus="appearFlag" @blur="loseBlur" readonly v-model="inputProvence"  suffix-icon="el-icon-caret-bottom" placeholder="省份"></el-input>
        <div v-if="inputFlag.seletProvinceFlag" class="chooseProvinceDetail">
          ...
          <div class="df j-around">
            <div>
              <div ref="provinceList" class="provinceList-item" :class="{'changeYellow'}" @click="seletProvince(item.name)" v-for="(item,index) in provinceList.slice(0,13)" :key="index">
                {{item.name}}
              </div>
            </div>
            <div>
              <div ref="provinceList" class="provinceList-item b1" @click="seletProvince(item.name)" v-for="(item,index) in provinceList.slice(13,26)" :key="index">
                {{item.name}}
              </div>
            </div>
            <div>

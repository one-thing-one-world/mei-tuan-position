       ...
       <el-input  @focus="appearFlag" @blur="loseBlur" readonly v-model="inputProvence"  suffix-icon="el-icon-caret-bottom" placeholder="省份"></el-input>
        <div v-if="inputFlag.seletProvinceFlag" class="chooseProvinceDetail">
          ...
          <div class="df j-around">
            ...
            <div>
              <div ref="provinceList" class="provinceList-item " @click="seletProvince(item.name,index)" v-for="(item,index) in provinceList.slice(13,26)" :key="index">
                {{item.name}}
              </div>
            </div>
            <div>
              <div class="provinceList-item" @click="seletProvince(item.name,index)" v-for="(item,index) in provinceList.slice(26)" :key="index">
                {{item.name}}
              </div>
            </div>
          </div>

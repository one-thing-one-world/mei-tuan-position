  ...
  <div class="box">
    ...
    <div>
      <div v-for="(item,index) in letterCityList" :key="index">
        <div class="letterCityList b1 df">
          <div class="letter-text-style"> <a  :id="`#${letterList[index]}`">{{letterList[index]}}</a></div>

          <div class="item-box b1 df">
            <div class="item-loop r1" v-for="(item1,index1) in item" :key="index1">
              {{item1.name}}
            </div>
          </div>
        </div>

...
<div>
  ...
  <div v-if="hostelContent.length>0">
  <div class="hostelContent" v-for="(item,index) in hostelContent" :key="index">
    <div class="itemPhotos" v-if="item.photos[1].url>0"> <img :src="item.photos[1].url" alt=""></div>
  </div>
  </div>
</div>
</template>
  ...
  methods: {
    triangleMouseEneter(item, index) {
      ...
      this.$api.getKeyWordSearch(this.positionCity, item.title).then(res => {
        this.hostelContent = res.data.pois.slice(0, 6)
        console.log(this.hostelContent);
      }).catch(err => {
        console.log(err);
      })
    },
